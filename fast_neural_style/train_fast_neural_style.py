...
tf.app.flags.DEFINE_string("CONTENT_IMAGES_PATH", None,
                           "Path to content image(s)")
...
def total_variation_loss(layer):
    ...
    y = tf.slice(layer, [0, 0, 0, 0], tf.pack(
        [-1, height - 1, -1, -1])) - tf.slice(layer, [0, 1, 0, 0],
                                              [-1, -1, -1, -1])
    x = tf.slice(layer, [0, 0, 0, 0], tf.pack(
        [-1, -1, width - 1, -1])) - tf.slice(layer, [0, 0, 1, 0],
                                             [-1, -1, -1, -1])
    return tf.nn.l2_loss(x) / tf.to_float(tf.size(x)) + tf.nn.l2_loss(
        y) / tf.to_float(tf.size(y))
...
def gram(layer):
    ...
    grams = tf.batch_matmul(
        filters, filters, adj_x=True) / tf.to_float(size / FLAGS.BATCH_SIZE)
...
def main(argv=None):
    ...
    # model is a residual network
    generated = model.net(images / 255.)

    net, _ = vgg.net(FLAGS.VGG_PATH, tf.concat(0, [generated, images]))
    content_loss = compute_content_loss(content_layers, net)
    style_loss = compute_style_loss(style_features_t, style_layers, net)
